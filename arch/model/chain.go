...
// `ChainLinks` is a struct that contains a slice of pointers to `Block`s and a pointer to the latest `Block`.
...
// `NewChain()` creates a new chain with a genesis block and returns a pointer to the chain
...
// This is a method that returns the length of the chain.
